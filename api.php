...
$configFile   = $securePath . "/config.json";
...
if (in_array($action, ["start", "pause"])) {
    ...
    $emptyRoutes = json_encode(["runner" => [], "hunter" => []], JSON_PRETTY_PRINT);

    if ($action === "start") {
        $state["status"] = "running";
        $state["round_start"] = time();
        file_put_contents($securePath . "/routes.json", $emptyRoutes);
        // Snapshots aus vorheriger Runde clearen
        file_put_contents($snapshotFile, json_encode(null));
        $hunterSnap = $securePath . "/hunter_snapshot.json";
        file_put_contents($hunterSnap, json_encode(null));
    } else {
        $state["status"] = "paused";
        file_put_contents($securePath . "/routes.json", $emptyRoutes);
        // Snapshots bei Pause clearen
        file_put_contents($snapshotFile, json_encode(null));
        $hunterSnap = $securePath . "/hunter_snapshot.json";
        file_put_contents($hunterSnap, json_encode(null));
    }
...
if ($action === "add_route") {
    ...
    $fp = fopen($routeFile, 'c+');
    flock($fp, LOCK_EX);
    $routes = json_decode(stream_get_contents($fp), true) ?? ["runner" => [], "hunter" => []];
    $routes[$type][] = ["lat" => (float)$lat, "lng" => (float)$lng];
    ftruncate($fp, 0);
    rewind($fp);
    fwrite($fp, json_encode($routes, JSON_PRETTY_PRINT));
    fflush($fp);
    flock($fp, LOCK_UN);
    fclose($fp);
